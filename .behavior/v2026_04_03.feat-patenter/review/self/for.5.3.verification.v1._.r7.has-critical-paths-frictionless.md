...
| step | command | expected | friction? |
|------|---------|----------|-----------|
| search | `rhx patent.priors.search --query "test"` | API key error with guidance | no — clear message |
| search invalid | `rhx patent.priors.search --query "ab"` | query too short error | no — min length explained |
| fetch | `rhx patent.priors.fetch --exid US12345678A1` | API key error with guidance | no — clear message |
| fetch invalid | `rhx patent.priors.fetch --exid INVALID` | format error with examples | no — shows correct format |

this path is frictionless: errors are clear, guidance is actionable.

#### critical path 2: propose → idea → research

| step | command | expected | friction? |
|------|---------|----------|-----------|
| propose | `rhx patent.propose` | route created with 9 stones | no — all templates copied |
| propose again | `rhx patent.propose` | error: already exists | no — shows path to extant route |
| propose outside git | `rhx patent.propose` | error: must be in git repo | no — clear constraint |

this path is frictionless: route creation is atomic, errors guide user.

#### critical path 3: search with good query

| scenario | behavior | friction? |
|----------|----------|-----------|
| query with API key | returns results | no — core flow works |
| query without API key | exit 2 with guidance | no — explains how to fix |
| query too short | exit 2 with min length | no — explains constraint |
| query too long | exit 2 with max length | no — explains constraint |

this path is frictionless: all validation paths have clear messages.

#### critical path 4: fetch complete document

| scenario | behavior | friction? |
|----------|----------|-----------|
| valid exid with API key | returns patent document | no — core flow works |
| valid exid without API key | exit 2 with guidance | no — explains how to fix |
| invalid exid format | exit 2 with format examples | no — shows correct format |
| patent not found | exit 2 with verification hint | no — suggests verify exid |

this path is frictionless: all paths provide actionable guidance.
...
the 4 updated snapshots are temporal (temp path timestamps), not behavioral.

### friction indicators

| indicator | present? |
|-----------|----------|
| unexpected errors | no |
| unclear messages | no |
| dead ends | no |
| obscure output | no |
| inconsistent behavior | no |

### conclusion

all 4 critical paths remain frictionless:
1. search errors guide user to API key registration
2. fetch errors show format examples
3. propose creates complete route atomically
4. all paths have actionable guidance
