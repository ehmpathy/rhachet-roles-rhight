...
ran `THOROUGH=true npm run test:integration`:
...
4 snapshots updated due to date change in route path (2026_04_03 → 2026_04_04). this is expected behavior — route directory name includes current date.

all 19 tests pass: **confirmed**

### why it holds

the test suite is deterministic and complete:

1. **no flakiness**: API-dependent tests (search, fetch) handle multiple valid outcomes:
   - with API key: verifies success output
   - without API key: verifies "API key required" error
   - both are assertions, not bypasses

2. **isolation**: each test uses `genTempDir({ git: true })` for fresh environment
   - no shared state between tests
   - no filesystem leakage
   - git repo initialized per test

3. **snapshot stability**: only date-based snapshots change
   - route path includes `v{date}` by design
   - date change is expected, not a regression

4. **no prior failures**: this is a new role
   - no inherited tech debt
   - no "already broken" tests to fix
   - clean slate

the 19 tests exercise:
- 7 search cases (help, no query, short query, valid, long, vague, unknown arg)
- 6 fetch cases (help, no exid, invalid format, valid, not found, unknown arg)
- 6 propose cases (route create, branch bind, extant route, invalid editor, help, no git)
